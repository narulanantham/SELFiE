...
def cos_lsf_arulanantham(mean_wave, ltp, pixel):
    ...
    #ltp = desired lifetime position (enter "LTP1", "LTP2", "LTP3", "LTP4"); dtype = string
    #pixel = return x-values as pixels or wavelength in Angstroms (enter "True" or "False"); dtype = string

    #LTP2, LTP3, and LTP4 dictionaries created in LSF_dictionaries.py
    ...
    if ltp == "LTP1":
        ...
        with open("cos_lsf_ltp4.pickle", "rb") as handle:
            file_table = pickle.load(handle,
                                     encoding='latin1')
    ...
    #### LTP2, LTP3, and LTP4 HAVE MULTIPLE FILES, SO DICTIONARY HAS EXTRA WAVELENGTH DIMENSION ##
    else:
        lsffiletopick = file_table["lsffiletopick"]
        laminwave = np.argmin(np.absolute(lsffiletopick[chan][0] - mean_wave))
...
def fcosx_function(wavelengths, nlines, polyorder, ltp, *a_set):
    ...
    if ltp == "LTP0":
        ...
        if ltp == "LTP1": #Need to increase length of LSF to match data
            ...
            yfinelsf = zoom(lsfy, reduction_factor)
